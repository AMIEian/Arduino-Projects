...
void node8_Tasks()
{
  ...
  {
    Serial.println("Node - 9");
    for (j = 0; j < 24; j++)
    {
      data[j] = node8.getResponseBuffer(j);
      Serial.print(data[j]);
      Serial.print(" ");
    }
    Serial.println("-------------------------------------");
  ...
  if (result==0)
    {
      ...
      if(node_8_old != temp[15])
        {
          Serial.println("New Data Received. Writting Data to Display");
          
          node8.setTransmitBuffer(0, temp[0]);
          node8.setTransmitBuffer(1, temp[1]);
          node8.setTransmitBuffer(2, temp[2]);
          
          node8.setTransmitBuffer(3, temp[3]);
          node8.setTransmitBuffer(4, temp[4]);
          node8.setTransmitBuffer(5, temp[5]);
        
          node8.setTransmitBuffer(6, temp[6]);
          node8.setTransmitBuffer(7, temp[7]);
          node8.setTransmitBuffer(8, temp[8]);
        
          node8.setTransmitBuffer(9, temp[9]);
          node8.setTransmitBuffer(10, temp[10]);
          node8.setTransmitBuffer(11, temp[11]);
        
          node8.setTransmitBuffer(12, temp[12]);
          node8.setTransmitBuffer(13, temp[13]);
          node8.setTransmitBuffer(14, temp[14]);
