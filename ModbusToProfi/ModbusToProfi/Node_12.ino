...
void node12_Tasks()
{
  ...
  {
    Serial.println("Node - 13");
    for (j = 0; j < 24; j++)
    {
      data[j] = node12.getResponseBuffer(j);
      Serial.print(data[j]);
      Serial.print(" ");
    }
    Serial.println("-------------------------------------");
  ...
  if (result==0)
    {
      ...
      if(node_12_old != temp[15])
        {
          Serial.println("New Data Received. Writting Data to Display");
          
          node12.setTransmitBuffer(0, temp[0]);
          node12.setTransmitBuffer(1, temp[1]);
          node12.setTransmitBuffer(2, temp[2]);
          
          node12.setTransmitBuffer(3, temp[3]);
          node12.setTransmitBuffer(4, temp[4]);
          node12.setTransmitBuffer(5, temp[5]);
        
          node12.setTransmitBuffer(6, temp[6]);
          node12.setTransmitBuffer(7, temp[7]);
          node12.setTransmitBuffer(8, temp[8]);
        
          node12.setTransmitBuffer(9, temp[9]);
          node12.setTransmitBuffer(10, temp[10]);
          node12.setTransmitBuffer(11, temp[11]);
        
          node12.setTransmitBuffer(12, temp[12]);
          node12.setTransmitBuffer(13, temp[13]);
          node12.setTransmitBuffer(14, temp[14]);
