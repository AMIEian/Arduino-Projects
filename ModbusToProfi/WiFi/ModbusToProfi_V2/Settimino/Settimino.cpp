...
S7Client::S7Client(int Media)
{
	...
	LastError = 0;
	PDULength = 0;
	RecvTimeout = 2000; // 500 ms
	if (Media==_S7WIFI)
	    TCPClient = new(WiFiClient);
	else
		TCPClient = new(EthernetClient);//new(UIPClient);//new(EthernetClient);
}
//-----------------------------------------------------------------------------
S7Client::~S7Client()
{
...
int S7Client::WriteArea(int Area, uint16_t DBNumber, uint16_t Start, uint16_t Amount, void *ptrData)
{
    ...
    while ((TotElements>0) && (LastError==0))
    {
        ...
        if (NumElements>MaxElements)
           NumElements=MaxElements;
		// If we use the internal buffer only, we cannot exced the PDU limitpp
