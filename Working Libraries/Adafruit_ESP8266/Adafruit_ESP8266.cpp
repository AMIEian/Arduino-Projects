...
boolean Adafruit_ESP8266::softReset(void) {
  ...
  if(find(F("ready"))) {           // Wait for boot message
 ...
 */
boolean Adafruit_ESP8266::httpPost(char *host, char* uri, char* data)
{
    /*
    *  POST uri HTTP/1.1\r\n
    *  Host: host\r\n
    ...
    */
	...
	char buffer[1024] = "POST "; // if your post request is longer than 199 bytes/characters then increase this value
	strcat(buffer,uri);
...
/*
boolean Adafruit_ESP8266::httpPost(Fstr *host, Fstr *uri, char* data)
{
    /*
    *  POST uri HTTP/1.1\r\n
    *  Host: host\r\n
