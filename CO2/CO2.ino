...
void checkPressureAndVolveOpen()
{
  ...
  if( (actualPressure <= (setPressure - 1)) && outputType == AIR ) //TURN ON MOTOR FOR FIX TIME SAY 1 SECOND
    {
      if((setPressure - actualPressure) <= 10)//changed from 6
        {
          timer = 200;//
        }
      else
        {
          timer = 800;  //SET TIMER FOR 1 SEC DELAY//chanded from 800
        }
  ...
  if( actualPressure >= (setPressure + 3) && hand_flag < 2000)
    {
      ...
      if(hand_flag >= 2000)
        {
          ...
          timer = 1000;  //TAKE NEXT READING AFTER 500 MS
        }
  ...
  if(actualPressure > setPressure)
    {
      ...
      if((actualPressure - setPressure) > 1 && (hand_flag < 2000))
        {
  ...
  if( outputType == CO_2 )
    {
      ...
      float temp_volumeCo2 = 0.0005 * temp_time;   //CALCULATE volumeCo2 OF C02 SUPPLIED DURING ON TIME OF VOLVE
