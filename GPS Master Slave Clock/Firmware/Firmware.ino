...
const byte interruptPin = 22;//25;

const int MinHrsData = 25;//22;
const int MinHrsShift = 27;//6;
const int MinHrsStore = 26;//7;
...
void setup()
{
  ...
  Wire.begin(5,23,100000);//Wire.begin(5,4,100000); //Wire.begin(SDA, SCL);
  ...
  ntpStarted = NTPServer.begin(123 , GetUTCTime );
  if(ntpStarted == true)
    Serial.println("NTP Started Successfully...!");
  else
    Serial.println("NTP Unable to Start...!");
  ...
  hws.begin( GPSBAUD ); //,SERIAL_8N1,16,17); // 13->RX , 15->TX
  ...
  while(1)
    {
      ...
      hrs = loc_time.hour;
      mins = loc_time.minute;
      secs = loc_time.second;
